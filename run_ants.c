...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2018/02/21 16:17:00 by myprosku          #+#    #+#             */
/*   Updated: 2018/02/21 16:17:00 by myprosku         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "includes/lemin.h"

char	*name_top(t_tops *top, int name)
{
	t_tops		*top_tmp;
...
void	move(t_uniq_path *ways, t_tops *tops, int *arr, t_ants *ants)
{
	...
	while (ants->number_a <= ants->count_ants)
	{
		...
		while (k <= ants->count_ants)
		{
			...
			{
				ft_printf("L%d-%s ", k, name_top(tops, (arr[k] = w_temp->a_way[j])));
				if (w_temp != NULL)
					break ;
			}
			k++;
